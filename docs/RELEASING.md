...
2. `npm ci && npm test`。
3. `./scripts/package-release.sh` 生成 `dist/harmony-offline-sdk-0.1.4.har`、未签名 Demo HAP、SHA256SUMS。
4. 用 HAR 替换临时验收工程的源码依赖，`ohpm install --all` 后完整编译 Demo。核对 HAR 包内的 README、CHANGELOG、LICENSE、Index.ets，且不含签名/业务配置。
5. 提交源码并创建 `v0.1.4` 标签；创建 GitHub Release，上传 HAR、未签名 HAP 和 SHA256SUMS。未签名 HAP 用于构建核对，真机请使用源码自行签名运行。

## OHPM

OHPM 使用本机加密密钥手动发布。SDK 的 `author` 必须包含姓名和网址或邮箱；本项目使用 GitHub 作者网址。可先运行 `ohpm prepublish dist/harmony-offline-sdk-0.1.4.har` 做本地包校验。申请可用包名/命名空间后，按照官方文档配置发布账号、公钥和 publish_id，在本机保存加密私钥，最后 `ohpm publish <har>`。账号、私钥和签名文件不得提交 Git。
